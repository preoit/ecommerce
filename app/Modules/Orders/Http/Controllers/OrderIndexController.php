...
class OrderIndexController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        $itemsByOrder = DB::table('order_items')->leftJoin('products', 'products.id', '=', 'order_items.product_id')
            ->whereIn('order_items.order_id', $orderIds)->orderBy('order_items.id')->get(['order_items.order_id', 'order_items.product_title', 'order_items.variant_name', 'order_items.quantity', 'order_items.unit_price', 'order_items.line_total', 'products.slug'])->groupBy('order_id');
        ...
        $orders = collect($paginator->items())->map(function (object $order) use ($itemsByOrder, $courierBookedOrderIds): array {
            ...
            return [
                ...
                'products' => $items->map(fn (object $item): array => ['name' => $item->product_title, 'variant' => $item->variant_name, 'quantity' => (int) $item->quantity, 'unitPrice' => (float) $item->unit_price, 'lineTotal' => (float) $item->line_total, 'slug' => $item->slug])->values(),
                ...
                'codSurcharge' => (float) ($order->cod_surcharge ?? 0), 'discount' => (float) ($order->discount_total ?? 0), 'total' => (float) $order->total,
