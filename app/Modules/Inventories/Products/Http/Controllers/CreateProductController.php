...
class CreateProductController extends Controller
{
    ...
    public function storeCategory(Request $request): JsonResponse
    {
        ...
        $slug = $baseSlug;
        $suffix = 2;
        while (Category::withTrashed()->where('slug', $slug)->exists() || Product::withTrashed()->where('slug', $slug)->exists() || BlogCategory::where('slug', $slug)->exists()) {
            $slug = $baseSlug.'-'.$suffix++;
        }
    ...
    public function update(Request $request, Product $product): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'slug' => ['nullable', 'string', 'max:180', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/', Rule::notIn(config('seo.reserved_slugs')), Rule::unique('products', 'slug')->ignore($product->id), Rule::unique('categories', 'slug'), Rule::unique('blog_categories', 'slug')],
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (filled($data['slug'] ?? null) && BlogCategory::where('slug', $data['slug'])->exists()) {
    ...
    private function syncVariants(Product $product, array $variants): void
    {
        ...
        if ($skus->duplicates()->isNotEmpty()) {
            throw ValidationException::withMessages(['variants' => 'Every variant SKU must be unique.']);
        }
        ...
        $conflictingSku = ProductVariant::withTrashed()
            ->whereIn('sku', $skus)->whereNotIn('id', $existingIds)->value('sku');
        if ($conflictingSku) {
            throw ValidationException::withMessages(['variants' => "Variant SKU {$conflictingSku} is already in use."]);
        }
        ...
        if ($variants) {
            $product->updateQuietly(['stock_quantity' => $product->variants()->where('is_active', true)->sum('stock_quantity')]);
        }
    }
