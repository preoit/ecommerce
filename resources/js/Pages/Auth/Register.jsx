...
export default function Register() {
    ...
    const submit = (event) => { event.preventDefault(); post(route('customer.register.store'), { onFinish: () => reset('password') }); };
    ...
    return <AuthLayout portal="customer"><Head title="Create customer account" />
        <h1 className="text-2xl font-black tracking-tight text-[#36354a]">Create customer account</h1><p className="mt-1 text-sm text-slate-500">Register to order faster and track every purchase.</p>
        ...
        <p className="mt-6 text-center text-sm text-slate-600">Already have an account? <Link href={route('customer.login')} className="font-medium text-[#6c5ce7]">Sign in instead</Link></p>
    </AuthLayout>;
}
