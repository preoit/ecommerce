...
export default function ListingHero({ title, description, brandNames = [], parent, children }) {
    return <header>
        <div className="relative isolate overflow-hidden rounded-3xl px-6 py-8 shadow-lg sm:px-10 sm:py-10" style={{ background: 'linear-gradient(115deg, #2e1065, #5b21b6 55%, #7c3aed)', color: '#fff' }}>
            ...
            <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-2 text-xs text-violet-100 sm:text-sm">
                <Link href="/" aria-label="Home" className="inline-flex items-center rounded p-1 text-white/80 transition hover:bg-white/10 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-white"><House aria-hidden="true" className="size-4" /></Link><ChevronRight className="size-3 text-white/40" />
                {parent && <><Link href={parent.href} className="font-medium text-violet-100 transition hover:text-white">{parent.name}</Link><ChevronRight className="size-3 text-white/40" /></>}
