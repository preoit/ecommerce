...
export default function CartDrawer({ open, onClose }) {
    ...
    return <div className={`fixed inset-0 z-[100] ${open ? 'pointer-events-auto' : 'pointer-events-none'}`} aria-hidden={!open}>
        ...
        <aside role="dialog" aria-modal="true" aria-labelledby="cart-drawer-title" className={`absolute inset-y-0 right-0 flex w-full max-w-[530px] flex-col bg-white transition-transform duration-300 ease-out ${open ? 'translate-x-0 shadow-[-18px_0_55px_rgba(15,23,42,.20)]' : 'translate-x-full shadow-none'}`}>
            ...
            {summary.items.length > 0 && <footer className="shrink-0 border-t border-slate-200 bg-white px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 shadow-[0_-12px_30px_rgba(15,23,42,.08)] sm:px-6"><div className="space-y-3 text-[15px]"><div className="flex items-center justify-between text-slate-700"><span className="font-semibold">Cart subtotal</span><b className="text-base text-slate-950">{money(summary.subtotal)}</b></div><div className="rounded-xl border border-violet-100 bg-violet-50/70 p-3.5"><div className="flex items-start justify-between gap-5"><span className="inline-flex items-center gap-2 font-bold text-slate-800"><span className="grid size-8 place-items-center rounded-lg bg-white text-violet-600 shadow-sm"><Truck className="size-[18px]"/></span>Delivery</span><div className="text-right"><p className="text-sm font-bold text-slate-800">Calculated at checkout</p>{settings.enabled && <p className="mt-1 text-[13px] font-semibold leading-5 text-slate-600"><span className="whitespace-nowrap">Inside Dhaka {money(settings.insideDhaka)}</span><span className="mx-1.5 text-violet-300">•</span><span className="whitespace-nowrap">Outside Dhaka {money(settings.outsideDhaka)}</span></p>}</div></div></div><div className="flex items-center justify-between border-t border-dashed border-slate-300 pt-3 text-lg font-black text-slate-950"><span>Total</span><span className="text-xl text-violet-700">{money(summary.subtotal)}</span></div></div><div className="mt-4 grid grid-cols-2 gap-3"><Link href={route('storefront.cart')} onClick={onClose} className="flex h-12 items-center justify-center rounded-xl border-2 border-violet-200 text-sm font-bold text-violet-700 transition hover:border-violet-600 hover:bg-violet-50">View Cart</Link><Link href={route('storefront.checkout')} onClick={onClose} className="flex h-12 items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-3 text-center text-sm font-bold text-white shadow-lg shadow-violet-200 transition hover:-translate-y-0.5">Proceed to Checkout<ArrowRight className="size-4"/></Link></div></footer>}
        </aside>
    </div>;
}
