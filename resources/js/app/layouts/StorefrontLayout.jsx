...
export default function StorefrontLayout({ children }) {
    ...
    useEffect(() => {
        ...
        const trackLink = makeLink('Track Order', auth?.user ? (auth.user.is_admin ? '/dashboard' : '/account') : '/login', 'mobile-menu-secondary-action');
        const loginLink = makeLink(auth?.user ? 'Account' : 'Login', auth?.user ? (auth.user.is_admin ? '/dashboard' : '/account') : '/login', 'mobile-menu-secondary-action');
