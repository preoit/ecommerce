...
export default function Home({ featuredProducts = [] }) {
    ...
    return <StorefrontLayout>
        ...
        <section id="products" className="mx-auto max-w-[1280px] px-4 py-12 sm:px-6 lg:px-8"><div className="flex items-end justify-between gap-4"><div className="flex items-start gap-3"><span className="mt-1 grid size-10 place-items-center rounded-xl bg-violet-100 text-violet-700"><ShoppingBag className="size-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-600">Products</p><h2 className="mt-1 text-2xl font-extrabold text-slate-950">Latest products</h2></div></div><Link href={route('storefront.products.index')} className="shrink-0 rounded-lg border border-violet-200 px-4 py-2 text-sm font-bold text-violet-700 transition hover:bg-violet-600 hover:text-white">View all</Link></div>{featuredProducts.length > 0 ? <div className="mt-7 grid grid-cols-1 gap-4 min-[400px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:gap-6">{featuredProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="mt-7 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-16 text-center text-sm font-semibold text-slate-500">No published products are available.</div>}</section>
    </StorefrontLayout>;
}
