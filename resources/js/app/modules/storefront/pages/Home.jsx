...
export default function Home({ featuredProducts = [] }) {
    ...
    return <StorefrontLayout>
        ...
        {hasHero && <section className="bg-white"><div className={`mx-auto grid max-w-[1280px] gap-4 px-4 py-5 sm:px-6 lg:px-8 ${primaryBanners.length && secondaryBanner ? 'lg:grid-cols-[2fr_1fr]' : 'grid-cols-1'}`}>{primaryBanners.length > 0 && <PrimaryHeroSlider images={primaryBanners} href={website?.heroPrimaryLink || '#products'} />}{secondaryBanner && <a href={website?.heroSecondaryLink || '#products'} className="group block overflow-hidden rounded-lg bg-slate-100"><img src={secondaryBanner} alt="Secondary promotion" className="aspect-[1.2/1] h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.015]" fetchPriority="high" /></a>}</div></section>}
