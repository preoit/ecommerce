...
function PrimaryHeroSlider({ images, href }) {
    ...
    return <div className="group relative overflow-hidden rounded-lg bg-slate-100"><a href={href} className="block h-full"><img key={images[active]} src={images[active]} alt={`Featured promotion ${active + 1}`} className="aspect-[2.5/1] h-full w-full animate-[heroFade_.35s_ease-out] object-cover transition-transform duration-500 group-hover:scale-[1.015]" fetchPriority={active === 0 ? 'high' : 'auto'} /></a>{images.length > 1 && <div className="absolute bottom-4 left-5 flex gap-2">{images.map((image, index) => <button type="button" key={image} onClick={() => setActive(index)} className={`size-2.5 rounded-full border border-white ${active === index ? 'bg-violet-600' : 'bg-white/90'}`} aria-label={`Show slide ${index + 1}`} />)}</div>}</div>;
}
...
function ProductCard({ product }) {
    ...
    return <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white transition duration-300 hover:-translate-y-1 hover:border-violet-200 hover:shadow-[0_16px_40px_rgba(30,41,59,0.10)]">
        <Link href={href} className="relative block aspect-square overflow-hidden bg-slate-50">
            {product.image ? <img src={product.image} alt={product.name} width="520" height="520" loading="lazy" className="h-full w-full object-contain p-5 transition duration-500 group-hover:scale-105" /> : <span className="grid h-full place-items-center text-slate-300"><ImageIcon className="size-12" strokeWidth={1.4} /></span>}
            <div className="absolute left-3 top-3 flex flex-col items-start gap-2">{product.discount > 0 && <span className="rounded-full bg-violet-600 px-2.5 py-1 text-[11px] font-bold text-white">-{product.discount}%</span>}{product.isNewArrival && <span className="rounded-full bg-slate-950 px-2.5 py-1 text-[11px] font-bold text-white">New</span>}</div>
        ...
        <div className="flex flex-1 flex-col p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{product.brand || product.category}</p><h3 className="mt-1.5 line-clamp-2 min-h-11 text-[15px] font-bold leading-[1.45] text-slate-900 transition group-hover:text-violet-700"><Link href={href}>{product.name}</Link></h3><div className="mt-3 flex flex-wrap items-baseline gap-2"><span className="text-lg font-extrabold text-violet-700">৳{Number(product.price).toLocaleString('en-BD')}</span>{product.discount > 0 && <span className="text-xs font-semibold text-slate-400 line-through">৳{Number(product.regularPrice).toLocaleString('en-BD')}</span>}</div><Link href={href} className="mt-4 inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white transition hover:bg-violet-600"><Eye className="size-4" /> View details</Link></div>
    </article>;
}

export default function Home({ featuredProducts = [] }) {
    const { website } = usePage().props;
    const primaryBanners = website?.heroPrimaryImages?.length ? website.heroPrimaryImages : [website?.heroPrimaryImage || 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1400&q=85'];
    const secondaryBanner = website?.heroSecondaryImage || 'https://images.unsplash.com/photo-1607082349566-187342175e2f?auto=format&fit=crop&w=800&q=85';
    return <StorefrontLayout>
        ...
        <section className="bg-white"><div className="mx-auto grid max-w-[1280px] gap-4 px-4 py-5 sm:px-6 lg:grid-cols-[2fr_1fr] lg:px-8"><PrimaryHeroSlider images={primaryBanners} href={website?.heroPrimaryLink || '#products'} /><a href={website?.heroSecondaryLink || '#products'} className="group block overflow-hidden rounded-lg bg-slate-100"><img src={secondaryBanner} alt="Secondary promotion" className="aspect-[1.2/1] h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.015]" fetchPriority="high" /></a></div></section>
