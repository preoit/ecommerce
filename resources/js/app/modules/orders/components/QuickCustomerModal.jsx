...
export default function QuickCustomerModal({ open, onClose, onSaved }) {
    ...
    const submit = async event => {
        ...
        try {
            const response = await fetch(route('orders.customers.store'), { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '' }, body: JSON.stringify(data) });
            const result = await response.json();
            if (!response.ok) { setErrors(result.errors || { form: result.message || 'Customer could not be saved.' }); return; }
            onSaved(result.customer); setData(empty); onClose();
