import OrderCourier from '@/app/modules/courier/OrderCourier';
    ...
    return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <ol className="mt-5">{orderSteps.map(([value, label], index) => {
            ...
            return <li key={value} className="relative flex min-h-16 gap-3 last:min-h-0">
                ...
                <div className="min-w-0 pb-5"><span className={active ? 'block text-sm font-bold text-violet-700' : 'block text-sm font-semibold text-slate-700 dark:text-slate-200'}>{label}</span><span className="mt-0.5 block text-xs text-slate-400">{active ? 'Current status' : reached ? 'Completed' : 'Upcoming'}</span>{active && value === 'confirmed' && <LabelButton order={order} processing={labelProcessing} onClick={openLabel} compact />}</div>
            </li>;
        })}</ol>
...
function PaymentDetails({ order, processing, updatePaymentStatus }) {
    const paid = String(order.paymentStatus).toLowerCase() === 'paid';
    return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4 dark:border-slate-800"><h2 className="flex items-center gap-2 font-bold"><CircleDollarSign className="size-5 text-violet-600" />Payment details</h2><span className={`rounded-full px-3 py-1 text-xs font-bold capitalize ring-1 ${badgeClass(order.paymentStatus)}`}>{order.paymentStatus}</span></div>
        <dl className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><dt className="text-slate-500">Order source</dt><dd className="font-bold capitalize">{order.source}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Payment method</dt><dd className="font-bold uppercase">{order.paymentMethod}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Products subtotal</dt><dd className="font-semibold">{money(order.subtotal)}</dd></div>{Number(order.discountTotal)>0&&<div className="flex justify-between text-emerald-700"><dt>Discount</dt><dd className="font-semibold">−{money(order.discountTotal)}</dd></div>}<div className="flex justify-between"><dt className="text-slate-500">{deliveryArea(order.deliveryZone)}</dt><dd className="font-semibold">{Number(order.shippingTotal) > 0 ? money(order.shippingTotal) : <span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">Free</span>}</dd></div>{order.codSurcharge > 0 && <div className="flex justify-between"><dt className="text-slate-500">COD charge</dt><dd className="font-semibold">{money(order.codSurcharge)}</dd></div>}<div className="flex justify-between border-t border-slate-200 pt-4 text-lg dark:border-slate-700"><dt className="font-black">Grand total</dt><dd className="font-black text-violet-700">{money(order.total)}</dd></div></dl>
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-slate-800"><div><p className="text-xs font-bold text-slate-600">Payment confirmation</p>{order.paymentStatusUpdatedAt && <p className="mt-1 text-[11px] text-slate-400">Last changed {order.paymentStatusUpdatedAt}</p>}</div><button type="button" disabled={processing} onClick={() => updatePaymentStatus(paid ? 'pending' : 'paid')} className={paid ? 'h-10 rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-600 transition-colors hover:bg-slate-50 disabled:opacity-50' : 'h-10 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white shadow-sm transition-colors hover:bg-emerald-700 disabled:opacity-50'}>{processing ? 'Updating...' : paid ? 'Mark as Pending' : 'Mark as Paid'}</button></div>
    </section>;
}
...
export default function OrderShow({ order }) {
    ...
    const updatePaymentStatus = paymentStatus => {
        if (!paymentStatus || paymentProcessing) return;
        setPaymentProcessing(true);
        router.patch(route('orders.payment-status.update', order.id), { payment_status: paymentStatus }, { preserveScroll: true, onFinish: () => setPaymentProcessing(false) });
    };
    const openLabel = () => {
    ...
    return <AdminLayout><Head title={`Order ${order.number}`} /><main className="mx-auto max-w-7xl space-y-5">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><Link href={route('orders.index')} className="inline-flex items-center gap-2 text-sm font-bold text-violet-700 hover:underline"><ArrowLeft className="size-4" />Back to orders</Link>{canUseLabel && <LabelButton order={order} processing={labelProcessing} onClick={openLabel} />}</div>
        <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-black tracking-tight text-slate-950 dark:text-white">Order {order.number}</h1><span className={`rounded-full px-3 py-1 text-xs font-bold ring-1 ${badgeClass(order.statusKey)}`}>{order.status}</span></div><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-500"><span className="inline-flex items-center gap-1.5"><Clock3 className="size-4 text-violet-500" />{order.date}</span><span className="inline-flex items-center gap-1.5"><ShoppingBag className="size-4 text-violet-500" />{order.items.length} {order.items.length === 1 ? 'item' : 'items'}</span>{order.shippingLabelGeneratedAt && <span className="inline-flex items-center gap-1.5 font-semibold text-emerald-600"><Check className="size-4" />Label generated</span>}</div></div><div className="w-fit rounded-xl bg-violet-50 px-5 py-3 lg:text-right"><p className="text-xs font-bold uppercase tracking-wide text-violet-500">Order total</p><strong className="mt-1 block text-2xl text-violet-700">{money(order.total)}</strong></div></div></header>
        ...
        <div className={`flex items-center gap-2 rounded-xl border px-4 py-3 text-sm ${order.phoneVerified ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300' : 'border-slate-200 bg-slate-50 text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400'}`}><span className="grid size-6 place-items-center rounded-full bg-white/70 dark:bg-slate-950">{order.phoneVerified ? <Check className="size-4"/> : <X className="size-4"/>}</span><b>{order.phone}</b><span>{order.phoneVerified ? 'Phone verified by OTP' : 'Phone not verified'}</span></div>
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-6">
                ...
                <PaymentDetails order={order} processing={paymentProcessing} updatePaymentStatus={updatePaymentStatus} />
                <OrderCourier orderId={order.id}/>
            </div>
            ...
            <aside className="space-y-5">
                ...
                <StatusCard order={order} processing={processing} updateStatus={updateStatus} labelProcessing={labelProcessing} openLabel={openLabel} />
            </aside>
        </div>
