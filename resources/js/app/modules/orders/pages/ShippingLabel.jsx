    ...
    return <><Head title={`${order.number} Shipping Label`} />
        ...
        <div className="label-workspace min-h-screen bg-slate-100 px-4 pb-10">
            <div className="label-toolbar sticky top-0 z-20 mx-auto mb-6 flex max-w-4xl items-center justify-between gap-4 border-b border-slate-200 bg-slate-100/95 py-4 backdrop-blur">
                ...
                <button type="button" onClick={() => window.print()} className="group inline-flex h-11 items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-5 text-sm font-bold text-white shadow-lg shadow-violet-200 transition duration-200 hover:-translate-y-0.5 hover:from-violet-700 hover:to-indigo-700 hover:shadow-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-violet-200"><Printer className="size-4 transition-transform group-hover:scale-110" />Print A5 Label</button>
            </div>
            <article className="shipping-label mx-auto min-h-[210mm] w-[148mm] max-w-full bg-white p-[10mm] text-slate-950 shadow-xl">
                ...
                <section className="border-b border-slate-300 py-5"><div className="flex items-center justify-between"><h3 className="text-sm font-black uppercase tracking-wide">Parcel contents</h3><span className="text-xs font-bold text-slate-500">{order.items.length} product lines</span></div><div className="mt-3 divide-y divide-slate-200">{order.items.map((item, index) => <div key={`${item.title}-${index}`} className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-5 gap-y-1 py-3 first:pt-1"><div className="min-w-0"><p className="text-sm font-black leading-5">{item.title}</p><p className="mt-1 text-[11px] font-semibold text-slate-500">SKU: {item.sku || 'N/A'}{item.variantName ? ` · ${item.variantName}` : ''} · Quantity {item.quantity}</p></div><div className="text-right"><p className="text-sm font-black">{money(item.lineTotal)}</p><p className="mt-1 text-[11px] font-semibold text-slate-500">{money(item.unitPrice)} × {item.quantity}</p></div></div>)}</div></section>
